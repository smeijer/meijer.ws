...
The alternative would be to create an helper method to not set the header value if it's empty, but that'll only work if you have full control over all the places where headers are set. Going the middleware route is safer for my use case.

```ts
function setHeader(headers: Headers, key: string, value?: string | null) {
  if (value === '' || value == null) return;
  headers.set(key, value);
}

setHeader(request.headers, 'x-api-key', apiKey);
setHeader(request.headers, 'x-user-id', userId);
```
